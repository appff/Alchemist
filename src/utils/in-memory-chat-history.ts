...
import { callLlm, DEFAULT_MODEL, getFastModel } from '../model/llm.js';
import { z } from 'zod';
...
export class InMemoryChatHistory {
  ...
  constructor(model: string = DEFAULT_MODEL, modelProvider: string = 'openai') {
    this.model = model;
  ...
  setModel(model: string, modelProvider?: string): void {
    this.model = model;
    if (modelProvider) this.modelProvider = modelProvider;
  }

  /**
   * Generates a brief summary of an answer for later relevance matching.
   * Uses the fast model variant to avoid wasting quota on the main model.
   */
...
Generate a brief 1-2 sentence summary of this answer.`;
    ...
    // Use fast model for summaries to avoid quota pressure on the main model
    const summaryModel = getFastModel(this.modelProvider, this.model);
