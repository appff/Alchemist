...
`.env` 파일에 설정하는 모든 환경 변수 목록입니다. **최소 하나의 LLM API 키**가 있어야 Dexter가 작동합니다.
...
Dexter가 시작되면 터미널에 대화형 인터페이스가 표시됩니다:
...
3. **Anthropic**: 권한 부여 후 표시되는 인증 코드를 Dexter에 붙여넣습니다
...
2. 원하는 모델을 다운로드합니다:
   ...
   ollama pull qwen2.5
   ```
3. Dexter에서 `/model` → Ollama 선택 → 로컬에 설치된 모델 목록이 표시됩니다
...
Dexter는 슬래시(`/`)로 시작하는 커맨드를 지원합니다. 입력창에 `/`를 입력하면 사용 가능한 커맨드 목록이 자동완성 팝업으로 표시됩니다.
...
Dexter의 AI 에이전트는 다양한 전문 도구를 자동으로 선택하여 사용합니다. 사용자가 질문하면 에이전트가 적합한 도구를 판단하고 호출합니다. 도구는 크게 4가지 카테고리로 분류됩니다.
...
Dexter는 로컬 파일 시스템에서 파일을 읽고, 쓰고, 수정할 수 있습니다. 주로 포트폴리오 관리 기능에서 사용됩니다.
...
스킬(Skill)은 Dexter 에이전트에게 **복잡한 분석 워크플로우를 단계별로 안내하는 상세 지침서**입니다.
...
| 우선순위 | 위치 | 용도 |
|---------|------|------|
| 1 (낮음) | `src/skills/` (내장) | Dexter에 기본 포함된 스킬 |
...
Dexter는 한국어 자연어를 이해하고 적절한 도구와 스킬을 자동으로 선택합니다.
...
Dexter의 포트폴리오 관리 시스템은 개인 투자자를 위한 전문급 포트폴리오 추적·분석 도구입니다. 마크다운 파일 기반으로 보유 자산, 거래 내역, 배당금을 체계적으로 기록하고, 성과 분석부터 세금 최적화까지 펀드 매니저 수준의 분석을 제공합니다.
...
Dexter 포트폴리오 시스템은 투자 포트폴리오를 **마크다운 파일**로 관리합니다. 별도의 데이터베이스나 외부 서비스 없이, 로컬 파일 시스템에 모든 데이터가 저장됩니다.
...
Dexter의 WhatsApp 게이트웨이는 스마트폰의 WhatsApp을 통해 Dexter 에이전트에 접근할 수 있게 해주는 기능입니다. 터미널 앞에 앉아있지 않아도, 이동 중에 WhatsApp 메시지로 금융 질문을 보내면 Dexter가 분석 결과를 답장으로 보내줍니다.
...
게이트웨이가 실행 중이면 WhatsApp에서 **자기 자신에게 메시지**(셀프 챗)를 보내는 방식으로 Dexter와 대화합니다.
...
**Step 3:** Dexter를 재시작하면 자동으로 스킬이 발견됩니다.
...
```bash
rm -rf ~/.dexter       # 주의: 모든 Dexter 데이터 삭제
bun start              # 다시 시작
```
...
> **면책 조항:** Dexter가 제공하는 모든 분석, 데이터, 의견은 정보 제공 목적이며, 투자 조언이 아닙니다. 모든 투자 결정은 본인의 판단과 책임 하에 이루어져야 합니다. 과거 성과는 미래 수익을 보장하지 않습니다.
